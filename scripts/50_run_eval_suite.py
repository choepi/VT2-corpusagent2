...
def render_protocol_b(pb: dict) -> None:
    # The "unsupported" rate is 0.000 for every family at this configuration (every claim is handed
    # the question's full cited-evidence set, so a candidate evidence sentence is always found) --
    # a column that is always zero conveys nothing, so it is stated as a constant in the caption
    # instead of rendered as a dead column. Per-family values are still in the underlying JSON
    # (outputs/eval_suite/protocol_b.json) for a rerun under a stricter evidence scope.
    rows = []
    for f in FAMILIES:
        r = pb["per_family"][f]
        if r["n_claims"] == 0:
            ...
            rows.append(f"{FAMILY_LABEL[f]} & {r['faithfulness']:.3f} & {r['contradiction']:.3f} \\\\")
    o = pb["overall"]
    rows.append("\\midrule")
    rows.append(f"\\textbf{{Overall}} & {o['faithfulness']:.3f} & {o['contradiction']:.3f} \\\\")
    body = "\n".join(rows)
    tex = (
        "% AUTO-GENERATED by scripts/50_run_eval_suite.py -- do not edit by hand.\n"
        "\\begin{table*}[t]\n\\centering\\small\n"
        ...
        "evidence; faithfulness = entailed/total, contradiction = NLI contradicts. The unsupported rate "
        "(no evidence sentence found) is 0.000 for every family at this configuration, by construction "
        "(every claim is handed the question's full cited-evidence set), so it is stated here rather "
        "than shown as a column of zeros. No gold answer or gold evidence is used.}\n"
        "\\label{tab:protocol-b-results}\n\\begin{tabular}{@{}lcc@{}}\n\\toprule\n"
        "\\textbf{Family} & \\textbf{Faith.} & \\textbf{Contra.} \\\\\n\\midrule\n"
        f"{body}\n\\bottomrule\n\\end{{tabular}}\n\\end{{table*}}\n"
    )
